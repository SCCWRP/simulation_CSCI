...
scores_plot <- function(site, station_list){
  ...
  data <-  summaried_data_with_models_to_csci(site, 
                                              station_list, 
                                              name_string = c("CSCI_mean",
                                                              "CSCI_quantile..3",
                                                              "CSCI_quantile..4"))
  summary_data <- data$summ
  model_data <- data$est
  
  ae = model_data$range_e[1]
  as = model_data$range_s[1]
  
  mean_data <- summary_data %>% 
    filter(!is.na(CSCI_mean))
  
  q3_data <- summary_data %>%
    filter(!(is.na(CSCI_quantile..3)))
  
  q4_data <- summary_data %>% 
    filter(!is.na(CSCI_quantile..4))
  
  CSCI_plot <- summary_data %>%
    ggplot() +
    geom_point(aes(Count, CSCI_mean)) +
    geom_point(aes(Count,CSCI_quantile..3), color = "blue",
              size = 0.4) +
    geom_point(aes(Count, CSCI_quantile..4), color = "blue",
              size = 0.4) +
    #geom_line(data = mean_data, aes(Count, Sphere, color = "Spherical"), size = 1) +
    geom_line(data = mean_data, aes(Count, Expo, color = "Exponential"), size = 1) + 
    #geom_line(data = q3_data, aes(Count, Sphere, color = "Spherical"), size = .5) +
    geom_line(data = q3_data, aes(Count, Expo, color = "Exponential"), size = .5) +
    #geom_line(data = q4_data, aes(Count, Sphere, color = "Spherical"), size = .5) +
    geom_line(data = q4_data, aes(Count, Expo, color = "Exponential"), size = .5) +
